...
with tf.device("/gpu:0"):
	...
	for i in range(len(summaryImage)):
		summaryImage[i] = tf.strings.as_string(summaryImage[i])
...
summaryWriter = tf.compat.v1.summary.FileWriter("summary_{0}/{1}".format(opt.group,opt.model))
