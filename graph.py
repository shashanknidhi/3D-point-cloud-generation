...
def encoder(opt,image): # [B,H,W,3]
	...
	def linearLayer(opt,feat,outDim,final=False):
		...
		fc = tf.linalg.matmul(feat,weight)+bias
		batchnorm = batchNormalization(opt,fc,type="fc")
		relu = tf.nn.relu(batchnorm)
		return relu if not final else fc
	with tf.compat.v1.variable_scope("encoder"):
		feat = image
		with tf.compat.v1.variable_scope("conv1"): feat = conv2Layer(opt,feat,96) # 32x32
		with tf.compat.v1.variable_scope("conv2"): feat = conv2Layer(opt,feat,128) # 16x16
		with tf.compat.v1.variable_scope("conv3"): feat = conv2Layer(opt,feat,192) # 8x8
		with tf.compat.v1.variable_scope("conv4"): feat = conv2Layer(opt,feat,256) # 4x4
...
def decoder(opt,latent):
	...
	def deconv2Layer(opt,feat,outDim):
		...
		resize = tf.image.resize(feat,[H*2,W*2],method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
	...
	with tf.compat.v1.variable_scope("decoder"):
		...
		with tf.compat.v1.variable_scope("fc1"): feat = linearLayer(opt,feat,1024)
		with tf.compat.v1.variable_scope("fc2"): feat = linearLayer(opt,feat,2048)
		with tf.compat.v1.variable_scope("fc3"): feat = linearLayer(opt,feat,4096)
		feat = tf.reshape(feat,[opt.batchSize,4,4,-1])
		with tf.compat.v1.variable_scope("deconv1"): feat = deconv2Layer(opt,feat,192) # 8x8
		with tf.compat.v1.variable_scope("deconv2"): feat = deconv2Layer(opt,feat,128) # 16x16
		with tf.compat.v1.variable_scope("deconv3"): feat = deconv2Layer(opt,feat,96) # 32x32
		with tf.compat.v1.variable_scope("deconv4"): feat = deconv2Layer(opt,feat,64) # 64x64
		with tf.compat.v1.variable_scope("deconv5"): feat = deconv2Layer(opt,feat,48) # 128x128
		with tf.compat.v1.variable_scope("pixelconv"): feat = pixelconv2Layer(opt,feat,opt.outViewN*4) # 128x128
...
def encoder_resnet(opt,image): # [B,H,W,3]
	...
	with tf.compat.v1.variable_scope("encoder"):
		feat = image
		with tf.compat.v1.variable_scope("conv1"): feat = conv2Layer(opt,feat,32) # 32x32
		with tf.compat.v1.variable_scope("conv2"): feat = conv2Layer(opt,feat,64) # 16x16
		with tf.compat.v1.variable_scope("conv3"): feat = conv2Layer(opt,feat,128) # 8x8
		with tf.compat.v1.variable_scope("conv4"): feat = conv2Layer(opt,feat,256) # 4x4
		feat = tf.reshape(feat,[opt.batchSize,-1])
		with tf.compat.v1.variable_scope("fc1"): feat = linearLayer(opt,feat,1024)
		with tf.compat.v1.variable_scope("fc2"): feat = linearLayer(opt,feat,256,final=True)
		latent = feat
	return latent
...
def decoder_resnet(opt,latent):
	...
	with tf.compat.v1.variable_scope("decoder"):
		feat = tf.nn.relu(latent)
		with tf.compat.v1.variable_scope("fc1"): feat = linearLayer(opt,feat,1024)
		with tf.compat.v1.variable_scope("fc2"): feat = linearLayer(opt,feat,4096)
		with tf.compat.v1.variable_scope("fc3"): feat = linearLayer(opt,feat,16384)
		feat = tf.reshape(feat,[opt.batchSize,4,4,-1])
		with tf.compat.v1.variable_scope("resblock1"): feat = residualBlock(opt,feat)
		with tf.compat.v1.variable_scope("deconv1"): feat = deconv2Layer(opt,feat,512) # 8x8
		with tf.compat.v1.variable_scope("resblock2"): feat = residualBlock(opt,feat)
		with tf.compat.v1.variable_scope("deconv2"): feat = deconv2Layer(opt,feat,256) # 16x16
		with tf.compat.v1.variable_scope("resblock3"): feat = residualBlock(opt,feat)
		with tf.compat.v1.variable_scope("deconv3"): feat = deconv2Layer(opt,feat,128) # 32x32
		with tf.compat.v1.variable_scope("resblock4"): feat = residualBlock(opt,feat)
		with tf.compat.v1.variable_scope("deconv4"): feat = deconv2Layer(opt,feat,64) # 64x64
		with tf.compat.v1.variable_scope("resblock5"): feat = residualBlock(opt,feat)
		with tf.compat.v1.variable_scope("deconv5"): feat = deconv2Layer(opt,feat,32) # 128x128
		with tf.compat.v1.variable_scope("pixelconv"): feat = pixelconv2Layer(opt,feat,opt.outViewN*4) # 128x128
		XYZ,maskLogit = tf.split(feat,[opt.outViewN*3,opt.outViewN],axis=3) # [B,H,W,3V],[B,H,W,V]
	return XYZ,maskLogit # [B,H,W,3V],[B,H,W,V]
...
# L1 loss
def l1_loss(input):
	return tf.compat.v1.reduce_sum(tf.abs(input))
# L1 loss (masked)
def masked_l1_loss(diff,mask):
	return l1_loss(tf.boolean_mask(diff,mask))
# sigmoid cross-entropy loss
def cross_entropy_loss(logit,label):
	return tf.compat.v1.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(logits=logit,labels=label))
